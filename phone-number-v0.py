...
# hfs -ls
# hfs -getmerge phone-numbers-out.out phone-numbers1.txt
...
def isValidNumber(x):
    ...
    if alpha.match(phone_number):
        return (row, 5, phone_number) # this is a semantic outlier, it has alphabetic chars
    if str_len < 5 or str_len > 15:
        return (row, 5, phone_number) # this is a semantic outlier, not possibly a phone number because of the length issues
    ...
    if not isValid and not isNull:
        if phone_number[0] == "1": 
            ...
            isValid = regex.match(phone_number)
            return (row, 1, phone_number) if isValid else (row, 3, phone_number)
        elif phone_number[0] == "+":
        ...
        return (row, 1, phone_number) if isValid else (row, 4, phone_number) if isNull else (row, 3, phone_number) 
...
if __name__ == "__main__":
    ...
    for col in detected_cols: 
        ...
        if results is None:
            results = sc.union([validation])
        else:
            results = sc.union([results, validation])

    formatted_results = results.sortByKey()
    formatted_results.saveAsTextFile(sys.argv[1] + 'parsed-phone-num_v0.out') 
